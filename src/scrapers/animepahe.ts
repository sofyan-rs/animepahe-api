...
class Animepahe {
  ...
  isRefreshingCookies: boolean;
  activeBrowser: BrowserLike | null;
  cloudflareSessionCookies:
    | {
        header: string;
    ...
    | null;
  currentKwikRequest: string | null;
  ...
  async refreshCookies() {
    ...
    try {
      if (!browser) {
        browser = (await launchBrowser()) as BrowserLike;
  ...
  async scrapeIframe(id, episodeId, url) {
    ...
    const PlayModelModule = require("../models/playModel");
    const PlayModel = PlayModelModule.default ?? PlayModelModule;
