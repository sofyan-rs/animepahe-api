...
import { getJsVariable } from '../utils/jsParser';
...
type ScrapedAnimeInfo = {
    ...
    themes: string[];
    demographic: string[];
    external_links: Array<{ name: string; url: string | undefined }>;
    genre: string[];
...
class AnimeInfoModel {
    ...
    static async getAnimeReleases(animeId: string, sort: string, page: number) {
        const results = (await Animepahe.getData("releases", { animeId, sort, page })) as ReleaseResult | null;
    ...
    static async scrapeInfoPage(pageHtml: string) {
        if (!pageHtml) {
        ...
        const animeInfo: ScrapedAnimeInfo = {
            ids: {
    ...
    static async scrapeRelationsSection($: CheerioRoot) {
        const relations = {};
    ...
    static async scrapeRecommendationsSection($: CheerioRoot) {
    ...
    static extractStatusFromText(text: string) {
